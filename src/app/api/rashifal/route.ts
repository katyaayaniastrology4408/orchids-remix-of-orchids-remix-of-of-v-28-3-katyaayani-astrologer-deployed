  ...
  try {
    ...
    const upsertData = rashifals.map((r: any) => {
      ...
      if (error) throw error;
            ...
            // Daily auto-send rashifal email to 1 random user
            ...
            if (!existingNotif) {
              try {
                ...
                if (validUsers.length > 0) {
                  ...
                  const sentCount = (result as any)?.success ? 1 : 0;
                ...
                await supabase.from('rashifal_notifications').insert({
                  ...
                  emailResult = { sent: true, totalUsers: sentCount, sentTo: randomUser.email };
              } else {
