    ...
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-[#0a0a0f] text-[#f5f0e8]' : 'bg-[#fdfbf7] text-[#4a3f35]'}`}>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-24">
        ...
        >
          ...
          {isMounted ? (language === 'gu' ? 'પાછા જાઓ' : language === 'hi' ? 'વાપસ जाएं' : 'Go Back') : 'Go Back'}
        </Button>
              ...
              <div className="lg:sticky lg:top-28 flex flex-col gap-6">
                ...
                <div className={`rounded-2xl border p-5 flex flex-col gap-4 ${theme === 'dark' ? 'bg-[#12121a] border-[#ff6b35]/20' : 'bg-[#fffdf9] border-[#ff6b35]/20'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-wider opacity-50 font-bold">
                        {isMounted ? (language === 'gu' ? 'લેખક' : language === 'hi' ? 'लेखक' : 'Author') : 'Author'}
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-wider opacity-50 font-bold">
                        {isMounted ? (language === 'gu' ? 'પ્રકાશિત' : language === 'hi' ? 'प्रकाशित' : 'Published') : 'Published'}
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] uppercase tracking-wider opacity-50 font-bold">
                        {isMounted ? (language === 'gu' ? 'વ્યૂ' : language === 'hi' ? 'व्यू' : 'Views') : 'Views'}
                      </p>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {isMounted ? (language === 'gu' ? 'આ પોસ્ટ શેર કરો' : language === 'hi' ? 'यह पोस्ट शेयर करें' : 'Share this post') : 'Share this post'}
                    </Button>
                          ...
                          >
                            <div className="flex items-center justify-between mb-2 px-1">
                              <span className="text-sm font-semibold">
                                {isMounted ? (language === 'gu' ? 'શેર કરો' : language === 'hi' ? 'શેર કરો' : 'Share this post') : 'Share this post'}
                              </span>
                            ...
                            <div className="space-y-1">
                              ...
                              <button onClick={handleCopyLink} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${theme === 'dark' ? 'hover:bg-white/10' : 'hover:bg-gray-100'}`}>
                                ...
                                {copied
                                  ? (isMounted && language === 'gu' ? 'લિંક કૉપી થઈ!' : isMounted && language === 'hi' ? 'लिंक कॉपी हुई!' : 'Link Copied!')
                                  : (isMounted && language === 'gu' ? 'લિંક કૉપી કરો' : isMounted && language === 'hi' ? 'लिंक कॉपी करें' : 'Copy Link')
                                }
                              </button>
                            </div>
                          </motion.div>
                ...
                <div className="hidden lg:block">
                  <Link href="/blog">
                    <Button variant="outline" className="w-full border-[#ff6b35]/30 text-[#ff6b35] hover:bg-[#ff6b35]/10">
                      <ArrowLeft className="w-4 h-4 mr-2" />
                      {isMounted ? (language === 'gu' ? 'વધુ બ્લોગ પોસ્ટ' : language === 'hi' ? 'और ब्लॉग પોસ્ટ' : 'More Blog Posts') : 'More Blog Posts'}
                    </Button>
                  </Link>
                </div>
              </div>
              ...
              {post.tags && post.tags.length > 0 && (
                <div className="mt-12 pt-8 border-t border-border">
                  <div className="flex items-center gap-2 mb-4 text-sm font-bold uppercase tracking-widest opacity-60">
                    ...
                    {isMounted ? (language === 'gu' ? 'ટૅગ્સ' : language === 'hi' ? 'टैग्स' : 'Tags') : 'Tags'}
                  </div>
              ...
              {relatedPosts.length > 0 && (
                <section className="mt-20 pt-12 border-t border-border">
                  <h3 className="font-[family-name:var(--font-cinzel)] text-2xl font-bold mb-8">
                    {isMounted ? (language === 'gu' ? 'સંબંધિત પોસ્ટ્સ' : language === 'hi' ? 'संबंधित पोस्ट' : 'Related Posts') : 'Related Posts'}
                  </h3>
              ...
              <div className="mt-12 text-center lg:hidden">
                <Link href="/blog">
                  <Button className="bg-[#ff6b35] hover:bg-[#ff8c5e]">
                    <ArrowLeft className="w-4 h-4 mr-2" />
                    {isMounted ? (language === 'gu' ? 'વધુ બ્લોગ પોસ્ટ જુઓ' : language === 'hi' ? 'और ब्लॉग પોસ્ટ देखें' : 'View More Blog Posts') : 'View More Blog Posts'}
                  </Button>
                </Link>
              </div>
            </div>
